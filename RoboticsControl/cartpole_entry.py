...
from robotics_control.pid import PIDController
...
def _make_controller(params: PIDParams) -> PIDController:
    """
    TODO: Implement the controller for the cartpole.
    You can edit the PIDParams class to add more parameters to the controller.

    Step-by-step implementation suggestion:
    1. Replace `PIDController(**kwargs)` with your concrete class, e.g.
       `CartPolePIDController(**kwargs)` from `robotics_control.pid`.
    2. In controller `compute(obs, dt)`:
       - parse `[x, x_dot, theta, theta_dot]`
       - compute angle PID term + cart PID term
       - clip force and map to discrete action {0,1}
    3. Keep `reset()` clearing integrals and previous-action memory.
    4. Keep the mapping consistent with env action semantics:
...
def _run_mode(args):
    ...
    params = PIDParams(
        # TODO: Replace with your tuned gains.
        # Tuning order recommendation:
        # 1) Set angle PD first (theta_kp/theta_kd).
        # 2) Add small x PD (x_kp/x_kd) for centering.
