"""Main PPO training script for PushT."""
...
from src.core.env import make_env, make_vec_env
from src.core.networks import PPOAgent
from src.core.ppo import PPO
from src.utils.logger import CSVLogger, ConsoleLogger, TensorboardLogger
from src.utils.plotting import plot_reward_curve


def parse_args():
    parser = argparse.ArgumentParser(description="Train PPO on PushT")
    parser.add_argument("--lr", type=float, default=None)
    parser.add_argument("--lr_schedule", type=str, default=None, choices=["none", "linear"])
    ...
    parser.add_argument("--epochs_per_update", type=int, default=None)
    ...
    parser.add_argument("--curriculum", type=int, default=None, help="1 for on, 0 for off")
...
def main():
    ...
    if args.epochs_per_update is not None:
        config.epochs_per_update = args.epochs_per_update
    ...
    if args.curriculum is not None:
        config.curriculum = bool(args.curriculum)
    ...
    # Output: outputs/ppo_<timestamp>/
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = os.path.join(args.output_dir, f"ppo_{timestamp}")
