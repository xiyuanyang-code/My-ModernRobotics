...
def homog_3d(xi, theta):
    ...
    if omega_norm == 0:
        ...
        # normalize
        w_unit = omega / omega_norm
        v_unit = v / omega_norm
        theta_unit = theta * omega_norm
        R = rotation_3d(w_unit, theta_unit)
        w_hat = skew_3d(w_unit)

        p = (np.eye(3) - R) @ (w_hat @ v_unit) + (
            np.outer(w_unit, w_unit) @ v_unit
        ) * theta_unit
